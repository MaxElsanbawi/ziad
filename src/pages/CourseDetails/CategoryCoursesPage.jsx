...
function CategoryCoursesPage() {
  ...
  if (error) {
    return <div className="text-center p-16 bg-[#F9FAFC]">خطأ: {error}</div>;
  }

  return (
    <div dir="rtl" className="py-32 about p-16 bg-[#F9FAFC]">
      <h1 className="hidden">دورات الفئة {categoryId}</h1>
      {courses.length > 0 ? (
        <ul className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 p-4 text-center">
          {courses.map((course) => (
            <Link
              key={course.CourseID}
            ...
            >
              <li className="border p-4 rounded-lg shadow-md hover:shadow-lg transition-shadow ">
                ...
                {/* Display Course Name */}
                <p className="mt-2 text-[#011B7D]">{course.CourseName}</p>
              </li>
            </Link>
          ))}
        </ul>
      ) : (
        <div>لا توجد دورات لهذه الفئة.</div>
      )}
              <WhatsAppButton/>
    </div>
  );
