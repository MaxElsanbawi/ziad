...
export default function ClientsImageUpload() {
  ...
  const fetchPartners = async () => {
    try {
      const response = await fetch("https://phpstack-1509731-5843882.cloudwaysapps.com/api/Clients");
      if (response.ok) {
  ...
  const handleDeletePartner = async (id) => {
    if (window.confirm('هل أنت متأكد من حذف هذا الشريك؟')) {
      try {
        const response = await fetch(`https://phpstack-1509731-5843882.cloudwaysapps.com/api/Clients/${id}`, {
          method: "DELETE",
        });
          ...
          <table className="min-w-full bg-white">
            <thead className="hidden md:table-header-group">
              <tr className="bg-gray-50">
                ...
                <th className="px-6 py-3 text-right text-sm font-semibold text-primary">اسم الشريك</th>
              ...
              {currentPartners.length > 0 ? (
                currentPartners.map((partner, index) => (
                  ...
                  >
                    ...
                    <td className="px-6 py-4 text-right text-sm text-gray-700 block md:table-cell">
                      <span className="font-semibold md:hidden">اسم الشريك: </span>
                      {partner.name}
                    </td>
                    <td className="px-6 py-4 text-right text-sm text-gray-700 block md:table-cell">
