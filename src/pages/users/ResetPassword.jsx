  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch(
        "https://backend.camels.center/api/auth/reset-password",
        requestOptions
      );
      ...
      if (result.message === "Password reset successfully") {
        ...
        setTimeout(() => {
          navigate("/LoginPage"); // Navigate to the login page after 5 seconds
        }, 5000); // 5000 milliseconds = 5 seconds
      } else {
        setErrorMessage(result.message || "An error occurred. Please try again."); // Set error message
      }
    } catch (error) {
