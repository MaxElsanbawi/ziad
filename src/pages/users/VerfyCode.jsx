...
const VerifyOTPPage = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    const raw = JSON.stringify({
      ...
      OTP: otp, // Use the OTP from state
    });
    ...
    try {
      const response = await fetch(
        "https://backend.camels.center/api/auth/verify-password-otp",
        requestOptions
      );
      ...
      if (
        ...
        setTimeout(() => {
          navigate("/ResetPassword"); // Navigate to the reset password page after 5 seconds
        }, 5000); // 5000 milliseconds = 5 seconds
      } else if (result.message === "Invalid or expired OTP") {
        setErrorMessage("Invalid or expired OTP. Please try again."); // Set error message
      } else {
